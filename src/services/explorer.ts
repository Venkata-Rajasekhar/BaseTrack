...
export const getTransactionCount = async (address: string): Promise<number> => {
  try {
    const response = await axios.get(`https://api.basescan.org/api?module=proxy&action=eth_getTransactionCount&address=${address}&tag=latest&apikey=U252APBFXEV3C17KXRGVG8U2GPMNGFQ8MY`);
...
export const getEtheriumCount = async (address: string): Promise<number> => {
  try {
    const response = await axios.get(`https://api.basescan.org/api?module=account&action=balance&address=${address}&tag=latest&apikey=XX4VRM4YYZJ7ZVAA7KGQTUB3MVG6W7MCPQ`);
...
export const getTransactionList = async (address: string): Promise<Transaction[]> => {
  const apiUrl = `https://api.basescan.org/api?module=account&action=txlist&address=${address}&startblock=0&endblock=99999999&sort=asc&apikey=7G6GC36HD1DZANNM7HSF1ZN41PGI1YB5DC`; // Replace with the actual API URL
